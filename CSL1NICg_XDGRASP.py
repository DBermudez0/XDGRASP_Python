...
def nufft_init(om,Nd, Jd,Kd, n_shift, kernel):
    ...
    st["p"] = csr_matrix((uu_1,(mm_1,kk_1)), shape=(M,int(np.prod(Kd))))#what function to use in python
    return st
...
def mtimes(a,bb):
    if a["adjoint"]==0:
        ...
        res = np.zeros((np.shape(bb)[0:3]), dtype = np.complex128)
        
        ress = np.zeros((np.shape(bb)[0],np.shape(bb)[1], np.shape(bb)[3]))
        
        for tt in range(0, np.shape(bb)[3]):
            for ch in range(0, np.shape(bb)[2]):
                    b = bb[:,:,ch,tt]*a["w"][:,:,tt]
                    b = np.reshape(np.swapaxes(b,0,1), (np.shape(b)[0]*np.shape(b)[1],1))
                    res[:,:,ch] = np.reshape(nufft_adj(b,a["st"][tt])/np.sqrt(np.prod(a["imSize"])), a["imSize"][0], a["imSize"][1])
...
nufft = MCNUFFT(k_u,np.double(w_u),mat_cont["b1"])
y = k_data_u
recon_cs= mtimes(nufft,y)
data_gridding = recon_cs/np.max(np.abs(recon_cs[:]))
tvweight_dim1 = np.max(np.abs(recon_cs[:]))*0.03

param = {E:nufft, y: y, TV_dim1: TV_Temp, TVWeight_dim1: tvweight_dim1, TVWeight_dim2: 0, nite:5, display:1}
